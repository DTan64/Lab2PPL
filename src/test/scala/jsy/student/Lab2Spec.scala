...
class Lab2Spec(lab2: Lab2Like) extends FlatSpec {
  ...
  "Or" should "return true if either or both expressions are true" in {
    ...
    assert(eval(Binary(Or, N(1234), B(false))) === N(1234))
  }
  ...
  "Plus" should "add two number values and return a number" in {
    ...
    assert(eval(Binary(Plus, B(true), S("12"))) === S("true12"))
    assert(e3 === N(3))
    assert(eval(Binary(Plus, S("2"), S("123"))) === S("2123"))
  }
  ...
  "Minus" should "subtract two number values and return a number" in {
    ...
    val e4 = eval(Binary(Minus, S("123"), e2))
    val e5 = eval(Binary(Minus, N(1), B(true)))
    val e6 = eval(Binary(Minus, N(1), S("")))
    assert(eval(Binary(Minus, S("1"), S("1"))) === N(0))
    //val e7 = eval(Binary(Minus, N(1), S("asdf")))
    assert(e3 === N(2))
    assert(e4 === N(122))
    assert(e5 === N(0))
    assert(e6 === N(1))
  ...
  "Div" should "divide two number values and return a number" in {
    ...
    assert(e3 === N(1.6))
    assert(eval(Binary(Div, N(2), N(0))) === N(Double.PositiveInfinity))
  }
  ...
  "Eq" should "return true if two numerical values are the same" in {
    ...
    assert(e3 === B(true))
    assert(eval(Binary(Eq, S("123"), N(123))) === B(true))
    assert(eval(Binary(Eq, N(123), S("123"))) === B(true))
    assert(eval(Binary(Eq, S("123"), N(123.0))) === B(true))
  } 
  ...
  it should "return false if two numerical values are not the same" in {
    ...
    val e2 = N(7)
    val e3 = eval(Binary(Eq, e1, e2))
    val e4 = eval(Binary(Eq, B(true), B(false)))
    assert(e3 === B(false))
    assert(e4 === B(false))
    assert(eval(Binary(Eq, S("asdf"), B(false))) === B(false))
    assert(eval(Binary(Eq, S("asdf"), S("asdfjkl"))) === B(false))
  }
  ...
  "Lt" should "return true if the first expression is less than the second" in {
    ...
    assert(e3 === B(true))
    assert(eval(Binary(Lt, B(false), S("1"))) === B(true))
    assert(eval(Binary(Lt, B(true), S("asdfasdf"))) === B(false))
  } 
  ...
  "Seq" should "execute the first expression, followed by the second, and should eval to the second expression" in {
    ...
    println(eval(Binary(Seq, Binary(Plus, N(1), N(2)), B(true))))
    assert(e3 === N(2))
  } 
